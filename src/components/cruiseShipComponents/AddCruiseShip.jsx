...
import { cruiseShipService } from "../../services/cruiseShipService.jsx";

export const AddCruiseShips = () => {
  ...
  useEffect(() => {
    if (tripId != undefined) {
      setIsFetching(true);
      const resp = cruiseShipService.getCruiseShip(tripId)
        .then((response) => {
    ...
    if (packageImageLinks != "" && packageImage != "") {
      try {
        ...
        if (packDetails[6]) {
          ...
          tripId
            ? cruiseShipService.updateCruiseShip(
                tripId,
                JSON.stringify(jsonObject, null, 2)
              )
            : cruiseShipService.createCruiseShip(
                JSON.stringify(jsonObject, null, 2)
              );
        }
    ...
    >
      <Box sx={{ display: "flex" }}>
        <Breadcrumbs aria-label="Bread crumbs" separator={<NavigateNextIcon />}>
          ...
          <Link to="/cruise-ships">Cruise Ships</Link>
          <Typography color="text.primary">Add Cruise Ship</Typography>
        </Breadcrumbs>
      </Box>
      {loading ? (
        <Modal open={true}>
