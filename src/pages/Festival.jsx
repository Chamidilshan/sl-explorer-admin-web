...
const [imgUrl3, setImgUrl3] = useState([]);
  ...
  const confirmDelete = async () => {
    try {
      ...
      setFestivals((prevFestivals) => prevFestivals.filter((festival) => festival._id !== festivalIdToDelete));
    } catch (error) {
    ...
    const handleClick = async () => {
      ...
      return (
        ...
        <Box sx={{ display: 'flex' }}>
         ...
         <Paper sx={{margin: '1%', pl: '10%',pr:'10%'}}>
          <div className='flex justify-end'>
            <Button onClick={functionAdd} variant="contained" color="primary">Add New Festival/Event</Button>
          </div>
          <div style={{margin: '1%'}}> 
          <TableContainer>
            <Table style={{border: '1px solid #D4D4D4', color: '#262626'}}>
              ...
              <TableBody >
              {festivals.slice(page*rowsPerPage, page*rowsPerPage + rowsPerPage).map((festival) => (
                  <TableRow key={festival._id} >
                    {/* <TableCell>{festival._id}</TableCell> */}
                    <TableCell style={{fontSize: '16px'}}>{festival.festivalName}</TableCell>
                    <TableCell style={{fontSize: '16px',width:'100px',}}>
                      <img src={festival.festivalCoverImage} alt={festival.festivalCoverImage} style={{ maxWidth: '100px' }} />
                    </TableCell>
                    <TableCell style={{fontSize: '16px'}}>{festival.festivalDescription}</TableCell>
                    <TableCell style={{fontSize: '16px',width:'100px',overflowWrap:'break-word'}}>{festival.festivalDate}</TableCell>
                    <TableCell style={{fontSize: '16px',width:'100px',overflowWrap:'break-word'}}>{festival.festivalTitle}</TableCell>
                    <TableCell style={{fontSize: '16px',width:'100px',}}>
                      <img src={festival.festivalImageLinks} alt={festival.festivalImageLinks} style={{ maxWidth: '100px' }} />
                    </TableCell>
                    <TableCell>
                    <Button variant='contained' onClick={e=>{handleEdit(festival._id)}} color='primary' sx={{margin: '5px'}} >Edit</Button>
                      <Button variant='contained' onClick={e=>{handleDelete(festival._id)}} style={{backgroundColor: '#D97706'}} >Delete</Button>
                    </TableCell>
                  </TableRow>
                ))} 
              </TableBody>
         ...
         <DialogContent> 
            <form onSubmit={handleSubmit}> 
              <Stack spacing={2} margin={2}>
                <TextField value={festivalName} required onChange={e=>{festivalNameChange(e.target.value)}} variant='outlined' label='Festival/Event Name'></TextField>
                <input type="file" onChange={handleFileChange}/> 
                {img !== '' ? (imgUrl && <img src={imgUrl} alt="Selected" />) : null} 
                    <br/>
                <TextField value={festivalDate} required onChange={e=>{festivalDateChange(e.target.value)}} variant='outlined' label='Festival/Event Date'></TextField>
                <TextField value={festivalDescription} required onChange={e=>{festivalDescriptionChange(e.target.value)}} variant='outlined' label='About Festival/Event'></TextField>
                <TextField value={festivalTitle} required onChange={e=>{festivalTitleChange(e.target.value)}} variant='outlined' label='Celebration Method'></TextField>
                <input type="file" onChange={handleFileChange}/> 
                {img !== '' ? (imgUrl && <img src={imgUrl} alt="Selected" />) : null} 
                    <br/>
